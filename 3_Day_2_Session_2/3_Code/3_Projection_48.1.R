...
# Load list of scenario input files
scens <- list.files("../2_Parameters/Setup_files/")

# Create a list of test values for each recruitment series. 

init<-c(seq(0,0.08, by=0.02), seq(0.085, 0.135, by=0.002),seq(0.14,0.3, by=0.05)) 
rec1<-c(seq(0,0.014, by=0.002),0.02,0.03, seq(0.04, 0.056, by=0.002),seq(0.1,0.3, by=0.05))
rec2<-c(seq(0,0.026, by=0.002),0.02,0.03, seq(0.04, 0.056, by=0.002),seq(0.1,0.3, by=0.05))
rec3<-c(seq(0,0.02, by=0.001),seq(0.05,0.3, by=0.05))
rec4<-c(seq(0,0.02, by=0.001),seq(0.05,0.3, by=0.05))
rec5<-c(0,seq(0.06,0.095, by=0.001),seq(0.1,0.3, by=0.05))
...
time<-tictoc::toc()
paste(round((time$toc-time$tic)/60,2), "minutes to run scenarios")
