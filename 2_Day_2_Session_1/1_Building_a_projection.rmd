...
To begin with we calculate the length based maturity at each time step, another way to think of it is, given the length of each age class at each time step, what proportion of the age class is mature. The `rampOgive()` function takes a matrix (in this case our lengths by age class), a mid point of the o-give, and a width - which dictates the slope of the o-give.
...
We now do essentially the exact same thing using the selectivity o-give. 
...
Whilst mathematically we can work out the exact number of krill in our simulated population, we cannot actually go out to the ocean and measure every krill. To simulate we have a parameter called B0logsd (which I plan to rename to FutureSurveyCV).

The Grym uses B0logSD to add variability to the calculated B0 at the beginning of each run. This is to account for the sampling and methodological error in surveys during the forward projections.
